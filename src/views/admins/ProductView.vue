<template>
    <div class="flex items-center justify-start mb-5">
        <button @click="addProduct()" class="w-max h-max rounded bg-blue-400 py-3 px-4 text-white font-[500]">ADD</button>
    </div>
            ...
            <modal v-if="showModal" @close="showModal = false">
                <template v-slot:content>
                    ...
                    <form @submit.prevent="submitForm">
                        ...
                        <div class="mb-4">
                            <label for="imageUrl" class="block text-sm font-medium text-gray-900">Brand</label>
                            <select v-model="formData.brandName"
                                class="block w-full mt-1 p-2 py-3 border rounded outline-none text-gray-900 ring-inset ring-gray-300 sm:text-sm sm:leading-6">
                                <option v-for="item in apiDataBrand" :key="item.id">{{ item.name }}</option>
                            </select>
                            <span class="text-red-600 text-[12px] font-[500]">{{ errorMessage.brandName }}</span>
                        </div>
                        <div class="mb-4">
                            <label for="imageUrl" class="block text-sm font-medium text-gray-900">Category</label>
                            <select v-model="formData.categoryName"
                                class="block w-full mt-1 p-2 py-3 border rounded outline-none text-gray-900 ring-inset ring-gray-300 sm:text-sm sm:leading-6">
                                <option v-for="item in apiDataCategory" :key="item.id">{{ item.name }}</option>
                            </select>
                            <span class="text-red-600 text-[12px] font-[500]">{{ errorMessage.categoryName }}</span>
                        </div>
    ...
    methods: {
        ...
        getDataBrand() {
            apiService.get("/Brand")
                .then((response) => {
                    this.apiDataBrand = response.data;
                })
                .catch((error) => {
                    console.error("Lỗi khi gửi yêu cầu API:", error);
                });
        ...
        getDataCategory() {
            apiService.get("/Category")
                .then((response) => {
                    this.apiDataCategory = response.data;
                })
                .catch((error) => {
                    console.error("Lỗi khi gửi yêu cầu API:", error);
                });
        ...
        submitForm() {
            ...
            const brandName = this.formData.brandName;
            const categoryName = this.formData.categoryName;
            if (!isNotEmpty(name)) {
                this.errorMessage.name = 'Hãy nhập tên sản phẩm.';
            } else if (!isNotEmpty(description)) {
                this.errorMessage.description = 'Hãy nhập mô tả sản phẩm.';
            } else if (!isNotEmpty(imageUrl)) {
                this.errorMessage.imageUrl = 'Hãy thêm ảnh cho sản phẩm';
            } else if (!isNotEmpty(brandName)) {
                this.errorMessage.brandName = 'Hãy chọn nhãn hàng.';
            } else if (!isNotEmpty(categoryName)) {
                this.errorMessage.categoryName = 'Hãy chọn loại sản phẩm.';
            } else {
                if (this.addtingItem) {
                    apiService.get("/Product/GetMaxProductId")
                        .then((response) => {
                            const maxId = response.data;
                            const newId = maxId + 1;

                            this.productObject.id = newId.toString();
                            this.productObject.name = this.formData.name;
                            this.productObject.description = this.formData.description;
                            this.productObject.imageUrl = this.formData.imageUrl;
                            this.productObject.brandName = this.formData.brandName;
                            this.productObject.categoryName = this.formData.categoryName;

                            apiService.post("/Product", this.productObject)
                                .then((response) => {
                                    this.productObject = {
                                        id: '',
                                        name: '',
                                ...
                                .catch((error) => {
                                    console.error("Error creating a new Product object:", error);
                                });
                        })
                        .catch((error) => {
                    ...
                    this.productObject.id = this.editingItemId;
                    this.productObject.name = this.formData.name;
                    this.productObject.description = this.formData.description;
                    this.productObject.imageUrl = this.formData.imageUrl;
                    this.productObject.brandName = this.formData.brandName;
                    ...
                    apiService.put(`/Product/${this.editingItemId}`, this.productObject)
                        .then((response) => {
                            this.productObject = {
                                id: '',
                                name: '',
                        ...
                        .catch((error) => {
                            console.error("Error updating the product object:", error);
                        });
                }
            }
        },
        ...
        deleteProduct() {
            apiService.delete(`/Product/${this.deleteId}`)
                .then(() => {
                    this.fetchData();
                })
                .catch((error) => {
                    console.error("Error deleting the Product object:", error);
                })
                .finally(() => {
