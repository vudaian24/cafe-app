<template>
    <div class="flex items-center justify-start mb-5">
        <button @click="addCategory()" class="w-max h-max rounded bg-blue-400 py-3 px-4 text-white font-[500]">ADD</button>
    </div>
                ...
                <template v-slot:content>
                    ...
                    <form @submit.prevent="submitForm">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea idx="description" v-model="formData.description"
                ...
                <tr class="bg-white border-b dark:bg-gray-80 dark:border-gray-50" v-for="item in apiData" :key="item.id">
                    ...
                    <td class="px-6 py-4">
                        <button class="font-medium text-blue-600 dark:text-blue-500 hover:underline mr-3"
                            @click="editCategory(item)">Edit</button>
...
export default {
    ...
    methods: {
        ...
        submitForm() {
            ...
            if (!isNotEmpty(name)) {
                this.errorMessage.name = 'Hãy nhập tên loại sản phẩm.';
            } else if (!isNotEmpty(description)) {
                this.errorMessage.description = 'Hãy nhập mô tả loại sản phẩm.';
            } else {
                if (this.addtingItem) {
                    apiService.get("/Category/GetMaxCategoryId")
                        .then((response) => {
                            const maxId = response.data;
                            const newId = maxId + 1;

                            this.categoryObject.id = newId.toString();
                            this.categoryObject.name = this.formData.name;
                            this.categoryObject.description = this.formData.description;

                            apiService.post("/Category", this.categoryObject)
                                .then((response) => {
                                    this.productObject = {
                                        id: '',
                                        name: '',
                    ...
                    this.categoryObject.id = this.editingItemId;
                    this.categoryObject.name = this.formData.name;
                    this.categoryObject.description = this.formData.description;

                    apiService.put(`/Category/${this.editingItemId}`, this.categoryObject)
                        .then((response) => {
                            this.categoryObject = {
                                id: '',
                                name: '',
        ...
        deleteCategory() {
            apiService.delete(`/Category/${this.deleteId}`)
                .then(() => {
                    this.fetchData();
                })
                .catch((error) => {
